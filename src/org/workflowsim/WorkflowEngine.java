...
//    	System.out.println(CloudSim.clock()+":"+CloudSim.getEntityName(ev.getSource())+"-> WorkflowEngine = "+ev.getTag());
	...
	/**
	 * Process a job return event. 数据中心处理完成以后
	 * ，工作流引擎调用processJobReturn方法：若任务处理失败，则调用ReclusteringEngine中的方法，以便重新执行此任务
	...
	private void init() throws Exception {
		for (Job job : jobList) {
			...
			for (Task task : job.getTaskList()) {
				...
				// System.out.println("task#"+task.getCloudletId()+"
		...
		for (Vm vm : getAllVmList()) {
			...
			// Datacenter.vmAllocationPolicy.getHost(vm).getVmScheduler()
...
//          			System.out.println("=======================================================第"+iterateNum+"次迭代前的最优适应度值bestFitness为："+bestFitness);
				...
				if (GASchedulingAlgorithm.children.size() != GASchedulingAlgorithm.popsize) {
					...
					if (GASchedulingAlgorithm.gmax != iterateNum) {
						...
						/*
						 * 重置参数，以便执行下一次迭代 （进化前，需要计算每个染色体被选中的概率，故设置了这些参数）
						 */
						initIndexForGA = 0;
...
//      					System.out.println("*************************************************************************按照最优调度方案执行");
	...
	private void completeEvolution() throws Exception {
	    double bestFitness = Double.MAX_VALUE;
	    ...
	    iterateNum++;
	    if (iterateNum < GASchedulingAlgorithm.gmax) {
	        // Prepare for the next generation
	        prepareNextGeneration(worstIndex);
	        init();
	        sendNow(this.getId(), CloudSimTags.CLOUDLET_SUBMIT, null);
	    } else {
	        // GA has completed all iterations
	        endTime = System.currentTimeMillis();
	        algorithmTime = endTime - getScheduler(0).startTime;
	        findBestSchedule = 1; // Proceed with the best scheduling plan
	        init();
	        sendNow(this.getId(), CloudSimTags.CLOUDLET_SUBMIT, null);
	    }
	}
	
	private void selectBetterChildren() {
	    for (int i = 0; i < fitnessForGATempChildren.length; i += 2) {
	        int selectedIndex = (fitnessForGATempChildren[i] < fitnessForGATempChildren[i + 1]) ? i : i + 1;
	        GASchedulingAlgorithm.fitness.add(fitnessForGATempChildren[selectedIndex]);
	        GASchedulingAlgorithm.children.add(GASchedulingAlgorithm.tempChildren.get(selectedIndex));
	    }
	    GASchedulingAlgorithm.tempChildren.clear();
	}
	
	private void prepareNextGeneration(int worstIndex) {
	    GASchedulingAlgorithm.fitness.clear();
	    GASchedulingAlgorithm.schedules.clear();
	    GASchedulingAlgorithm.probSegments.clear();
	    GASchedulingAlgorithm.probs.clear();
	    GASchedulingAlgorithm.tempParents.clear();
	    for (int i = 0; i < GASchedulingAlgorithm.children.size(); i++) {
	        if (i == worstIndex) {
	    ...
	    GASchedulingAlgorithm.children.clear();
	    initIndexForGA = 0;
	    FogBroker.initIndexForGA = 0;
	    tempChildrenIndex = 0;
	    FogBroker.tempChildrenIndex = 0;
	 ...
	 * (non-Javadoc)
	 * 
	 * @see cloudsim.core.SimEntity#startEntity() Here we creata a message when it
	 * is started
	 */
	@Override
	...
	class EliteDecision {
		...
		public EliteDecision(double time, double energy, double cost, double fitness) {
			this.time = time;
			this.energy = energy;
			this.cost = cost;
	...
	// Assuming there's a list of elite decisions available
	List<EliteDecision> eliteDecisions = new ArrayList<>();
	...
	protected void processJobReturnForDBPGA(SimEvent ev) throws Exception {
	    Job job = (Job) ev.getData();
	    getJobsReceivedList().add(job);
	    jobsSubmitted--;
	    if(getJobsList().isEmpty() && jobsSubmitted == 0) {
	        if(findBestSchedule == 1) {
	            // Execute according to the best scheduling plan, then end this simulation
	            getController().updateExecutionTime();
	            sendNow(getSchedulerId(0), CloudSimTags.END_OF_SIMULATION, null);
	        }
	        if(initIndexForGA != GASchedulingAlgorithm.popsize) {
	            // Processing a particle (initially obtained particle)
	            already = 1;
	            FogBroker.initIndexForGA++;
	            fitnessForGA[initIndexForGA++] = caculateDBPGAfitness();
	            init();
	            if(initIndexForGA == GASchedulingAlgorithm.popsize) {
	                gaFlag = 1;
	                gaFlag2 = 0;
	                ...
	                GASchedulingAlgorithm.bestParent = GASchedulingAlgorithm.schedules.get(bestIndex);
	                GASchedulingAlgorithm.bestParentFitness = bestFitness;
	                GASchedulingAlgorithm.gbestSchedule = GASchedulingAlgorithm.bestParent;
	                updatebest.add(bestFitness);
	                for(int i = 0; i < fitnessForGA.length; i++)
	                    GASchedulingAlgorithm.probs.put(i, fitnessForGA[i] / totalFitness);
	                GASchedulingAlgorithm.getSegments();
	                init();
	            }
	            sendNow(this.getId(), CloudSimTags.CLOUDLET_SUBMIT, null); 
	            ...
	            gaFlag2 = 1;
	            fitnessForGATempChildren[tempChildrenIndex++] = caculateDBPGAfitness();
	            FogBroker.tempChildrenIndex++;
	            if(tempChildrenIndex == 4) {
	                gaFlag2 = 0;
	                tempChildrenIndex = 0;
	                FogBroker.tempChildrenIndex = 0;
	                // Select better children based on fitness
	                selectBetterChildren();
	                if(GASchedulingAlgorithm.children.size() != GASchedulingAlgorithm.popsize) {
	                    init();
	                    sendNow(this.getId(), CloudSimTags.CLOUDLET_SUBMIT, null); 
	                }
	                else if(findBestSchedule == 0){
	                    // Completed one evolution
	                    completeEvolution();
	                }
	            }
	        }
	...
	public double caculateDBPGAfitness() {
		Controller controller = (Controller) CloudSim.getEntity(this.getcontrollerId());
		controller.updateExecutionTime();
		double energy = controller.TotalEnergy; // 单位J焦
		double time = controller.TotalExecutionTime;
		...
		double MS_norm = time; // getMaxTimeInPopulation(); // or some predefined max value
		double TE_norm = energy; // getMaxEnergyInPopulation(); // or some predefined max value
		double TC_norm = cost; // getMaxCostInPopulation(); // or some predefined max value

		// Weights for the objectives
		double w1 = 0.33; // weight for time
		double w2 = 0.33; // weight for energy
		double w3 = 0.34; // weight for cost

		// Calculate fitness
		double Fi = w1 * MS_norm + w2 * TE_norm + w3 * TC_norm;

		// Adjust fitness based on distance to elite decisions and dominance
		Fi = adjustFitnessBasedOnElites(Fi, time, energy, cost);

		return Fi;
	}

	// Function to adjust fitness based on the closest elite decision
	public double adjustFitnessBasedOnElites(double fitness, double time, double energy, double cost) {
		EliteDecision closestElite = findClosestEliteDecision(time, energy, cost); 
		if (closestElite != null) {
				double distance = calculateDistance(closestElite, new EliteDecision(time,energy, cost, fitness)); 
				fitness *= (1.0 / (1.0 + distance)); 
				} 
		else {
				eliteDecisions.add(new EliteDecision(time, cost, energy, fitness)); 
				} 
		return fitness; 
		}
	
	// Function to find the closest elite decision to the current decision
	public EliteDecision findClosestEliteDecision(double time, double energy, double cost) {
		EliteDecision closest = null;
		double minDistance = Double.MAX_VALUE;
		for (EliteDecision elite : eliteDecisions) {
			double distance = calculateDistance(elite, new EliteDecision(time, energy, cost, 0));
	...
	// Function to calculate the Euclidean distance between two decisions
	public double calculateDistance(EliteDecision d1, EliteDecision d2) { 
		double timeDiff = d1.time - d2.time; 
		double energyDiff = d1.energy - d2.energy;
		double costDiff = d1.cost - d2.cost;
